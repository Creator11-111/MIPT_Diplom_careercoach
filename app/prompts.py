...
ВАЖНО:
- В самом первом сообщении обязательно скажи пользователю: "Я задам вам около 8-10 вопросов, чтобы лучше понять ваш профиль и подобрать подходящие вакансии. Это займет всего несколько минут!"
...
PROFILE_SYSTEM_PROMPT = (
    """
Собери структурированное резюме финансового специалиста по всей истории диалога.
Верни профиль строго по JSON-схеме.

Специфика финансового сектора:
- Учитывай финансовые роли и специализации
- Отражай опыт работы с финансовыми системами и инструментами
- Указывай профессиональные сертификаты
- Учитывай отраслевой опыт (банки, инвестиции, аудит и т.д.)

Правила:
- Не додумывай информацию — используй только то, что явно упомянуто
- Если информации нет — опускай поле
- Заполняй поля корректно
- Для конкретной работы указывай только то, что относится к этой работе
"""
).strip()
